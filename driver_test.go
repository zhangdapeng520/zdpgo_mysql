...
// go test "-ldflags=-X github.com/zhangdapeng520/zdpgo_mysql.driverNameTest=custom"
...
// See https://github.com/zhangdapeng520/zdpgo_mysql/wiki/Testing
func init() {
...
func TestRawBytes(t *testing.T) {
	runTestsParallel(t, dsn, func(dbt *DBTest, _ string) {
		...
		if rows.Next() {
			...
			// https://github.com/zhangdapeng520/zdpgo_mysql/issues/765
...
func TestErrorInMultiResult(t *testing.T) {
	...
	// https://github.com/zhangdapeng520/zdpgo_mysql/issues/1361
	var db *sql.DB
	if _, err := ParseDSN(dsn); err != errInvalidDSNUnsafeCollation {
		db, err = sql.Open("mysql", dsn)
		if err != nil {
